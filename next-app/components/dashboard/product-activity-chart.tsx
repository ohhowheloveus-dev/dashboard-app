...
const chartData = [
  ...
  { category: "processDelivery", value: 98000, fill: "var(--color-processDelivery)" },
  { category: "deliveryDone", value: 140000, fill: "var(--color-deliveryDone)" },
          ...
          {chartData.map((d) => (
            <div key={d.category} className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-2">
                <span className="h-2 w-2 rounded-full" style={{ backgroundColor: chartConfig[d.category as keyof typeof chartConfig].color }} />
                <span className="text-muted-foreground">{chartConfig[d.category as keyof typeof chartConfig].label}</span>
              </div>
